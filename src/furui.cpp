...
void FurUI::initializeEngine(QQmlEngine *engine, const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("FurUI"));
...
void FurUI::registerTypes(const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("FurUI"));

    qmlRegisterSingletonType<ThemeManager>("FurUI.Core", 1, 0, "ThemeManager", [](QQmlEngine *engine, QJSEngine *scriptEngine) -> QObject * {
        Q_UNUSED(engine)
...
QUrl FurUI::componentUrl(const QString &fileName) const
{
    return QUrl(QStringLiteral("qrc:/furui/kit/") + fileName);
}
